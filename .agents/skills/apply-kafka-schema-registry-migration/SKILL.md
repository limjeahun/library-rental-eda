...
- 작업 중 변경된 Schema Registry, Kafka topic, schema contract, Gradle, Docker Compose, application.yml 설정은 `README.md`에 함께 반영
...
4. 실제 작업 중 수정한 topic 이름, schema 파일, generated package, serializer/deserializer 설정, Docker Compose 서비스, 실행/검증 명령은 누락 없이 `README.md`에 반영한다.
...
- README의 실행 방법, Docker Compose 구성, Schema Registry URL, 검증 명령이 이번 변경과 일치한다.
