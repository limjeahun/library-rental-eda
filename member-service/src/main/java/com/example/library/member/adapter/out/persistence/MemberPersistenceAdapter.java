...
public class MemberPersistenceAdapter implements LoadMemberByIdNamePort, LoadMemberPort, SaveMemberPort {
    ...
    /**
     * 회원 도메인 모델을 JPA 엔티티로 변환해 저장한 뒤 다시 도메인 모델로 반환.
     *
     * @param member 저장하거나 응답으로 변환할 회원 도메인 모델.
     * @return 저장 후 식별자와 최신 포인트가 반영된 회원 도메인 모델을 반환.
     */
    @Override
    ...
    public Optional<Member> loadMember(long memberNo) {
        return repository.findById(memberNo).map(mapper::toDomain);
    }

    /**
     * 회원 ID로 JPA 엔티티를 조회하고 도메인 모델로 변환.
     *
     * @param idName 대상 회원의 ID와 이름을 담은 공통 값 객체.
     * @return 회원 ID에 해당하는 회원 도메인 모델을 반환.
     */
    @Override
    public Optional<Member> loadMemberByIdName(MemberIdentity idName) {
        return repository.findByMemberId(idName.id()).map(mapper::toDomain);
    }
}
