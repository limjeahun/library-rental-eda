...
- `rental-service` `localhost:8080`
- `book-service` `localhost:8081`
- `member-service` `localhost:8082`
- `bestbook-service` `localhost:8084`
- Kafka 초기 토픽 생성 컨테이너

Kafka 지표, Spring Boot 서비스 지표, 컨테이너 로그 모니터링은 Prometheus/Loki/Grafana로 함께 올라옵니다. Grafana 기본 계정은 `admin` / `admin`이며, 기본 UI 언어는 한국어로 설정됩니다. `도서관 대여 EDA / 도서관 Kafka 모니터링`, `도서관 대여 EDA / 도서관 로그 모니터링` 대시보드가 자동 생성됩니다. 자세한 내용은 [Kafka Monitoring](docs/kafka-monitoring.md)을 참고합니다.

### 2. 서비스만 로컬에서 실행하기

기본 실행 방식은 Docker Compose입니다. 서비스 코드를 IDE나 Gradle로 직접 실행하려면 먼저 같은 포트를 쓰는 서비스 컨테이너를 내립니다.

```bash
docker compose stop member-service book-service rental-service bestbook-service
```
