...
/**
 * 도서 등록, 조회, AVAILABLE/UNAVAILABLE 상태 변경 흐름을 조율하는 application service.
 */
@Service
@Transactional
...
public class BookService implements AddBookUseCase, BookQueryUseCase, MakeAvailableBookUseCase, MakeUnavailableBookUseCase {
     ...
     * 새 도서를 입고 상태로 등록.
     *
     * @param command 등록할 도서 제목, 설명, 분류, 위치를 담은 application command 입니다.
     * @return 등록된 도서의 번호, 제목, 설명, 분류, 상태, 위치를 담은 결과 DTO 를 반환합니다.
     */
    @Override
     ...
     * @return 도서 번호에 해당하는 도서 결과 DTO 를 반환합니다.
     */
    @Override
    @Transactional(readOnly = true)
